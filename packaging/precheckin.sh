...
cat >> baselibs.conf << EOF
  ...
  targettype ${i} post "     ln -s \${binary} /usr/libexec/gcc/${i}-tizen-linux-gnueabi/\${GCCVER}/\${bin}"
  targettype ${i} post "   fi "
  targettype ${i} post " done "
  targettype ${i} post " ln -sf /usr/bin/ld /usr/libexec/gcc/${i}-tizen-linux-gnueabi/\${GCCVER}/ld"
  targettype ${i} post " ln -sf /usr/bin/ld.bfd /usr/libexec/gcc/${i}-tizen-linux-gnueabi/\${GCCVER}/ld.bfd"
  ...
  targettype ${i} preun "     rm /usr/libexec/gcc/${i}-tizen-linux-gnueabi/\${GCCVER}/\${bin}"
  targettype ${i} preun "   else "
  targettype ${i} preun "     echo "\${binary}.orig-arm not present !" "
  targettype ${i} preun "   fi "
  targettype ${i} preun " done "
  targettype ${i} preun " rm -f /usr/libexec/gcc/${i}-tizen-linux-gnueabi/\${GCCVER}/ld"
  targettype ${i} preun " rm -f /usr/libexec/gcc/${i}-tizen-linux-gnueabi/\${GCCVER}/ld.bfd"
